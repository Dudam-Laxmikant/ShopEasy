...
const Header = () => {
    ...
    const searchCategories = [
        { label: 'Browsing', options: ['All', 'Fashion'] },
        { label: 'Men\'s Fashion', options: ['T-shirts & Polos', 'Shirts', 'Jeans', 'Innerwear'] },
        { label: 'Women\'s Fashion', options: ['Western Wear', 'Ethnic Wear', 'Lingerie & Nightwear', 'Top Brands'] },
        { label: 'Electronics', options: ['Headphones', 'Smart Watches', 'Cameras', 'Laptops'] }
    ];
            ...
            <div>
                ...
                <ul className="space-y-1">
                    {[
                        ...
                        { name: 'My Payments', path: '/account/payments' },
                        ...
                        <li key={item.name}>
                            <Link
                                to={item.path}
                                onClick={closeMenu}
                                className="text-gray-700 hover:bg-gray-50 hover:text-yellow-700 cursor-pointer flex justify-between items-center px-2 py-3 rounded-lg text-[15px] font-medium transition-colors no-underline"
                            >
                                {item.name}
                            </Link>
                        </li>
                    ...
                    <div className="space-y-1">
                        {items.map((item) => (
                            <button
                                key={item}
                                onClick={() => {
                                    navigate(`/products?category=${title.split("'")[0]}`);
                                    closeMenu();
                                }}
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
                            ...
                            <div className="flex items-center justify-between w-full md:w-auto">
                                ...
                                <div className="flex items-center gap-4 md:hidden">
                                    ...
                                    <button
                                        ...
                                        className="relative p-2 text-gray-700 hover:text-yellow-600 transition-colors group border-none bg-transparent cursor-pointer"
                                    >
                                        <ShoppingCart className="h-6 w-6" />
                                        {cartCount > 0 && (
                                            <span className="absolute top-0 right-0 h-4 min-w-[16px] px-1 bg-red-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center animate-bounce">
                                                {cartCount}
                                            </span>
                                        )}
                                    </button>
                                ...
                                <div className="flex w-full bg-white rounded-md overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-yellow-600 transition-shadow">
                                    ...
                                    <div className="relative hidden sm:block border-r border-gray-200">
                                        <select
                                            ...
                                            className="h-full bg-gray-50 text-sm text-gray-700 px-3 py-2 pr-8 outline-none cursor-pointer hover:bg-gray-100 border-none appearance-none font-medium w-fit min-w-[120px]"
                                        >
                            ...
                            <div className="hidden md:flex items-center gap-4">
                                            ...
                                            <div className="py-2">
                                                <div className="px-4 py-2">
                                                    ...
                                                    <ul className="space-y-1 text-sm text-gray-600 list-none p-0">
                                                        <li><Link to="/account/orders" onClick={() => setIsAccountDropdownOpen(false)} className="block hover:text-yellow-600 hover:underline no-underline text-gray-600">Your Orders</Link></li>
                                                        <li><Link to="/account/wishlist" onClick={() => setIsAccountDropdownOpen(false)} className="block hover:text-yellow-600 hover:underline no-underline text-gray-600">Your Wish List</Link></li>
                                                        <li><Link to="/account/recently-viewed" onClick={() => setIsAccountDropdownOpen(false)} className="block hover:text-yellow-600 hover:underline no-underline text-gray-600">Recently Viewed</Link></li>
                                                        <li><Link to="/account/addresses" onClick={() => setIsAccountDropdownOpen(false)} className="block hover:text-yellow-600 hover:underline no-underline text-gray-600">Your Addresses</Link></li>
                                                        <li><Link to="/account/payments" onClick={() => setIsAccountDropdownOpen(false)} className="block hover:text-yellow-600 hover:underline no-underline text-gray-600">Payment Methods</Link></li>
                                                    </ul>
                                ...
                                <div
                                    id="cart-icon-target"
                                    onClick={() => setIsCartOpen(true)}
                                    className="relative flex items-end p-2 cursor-pointer hover:bg-yellow-500 rounded transition-colors group"
                                >
                                    <button className="relative p-2 text-gray-700 border-none bg-transparent cursor-pointer">
                                        <ShoppingCart className="h-6 w-6" />
                                        {cartCount > 0 && (
                                            <span className="absolute top-0 right-0 h-4 min-w-[16px] px-1 bg-red-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center animate-bounce">
                                                {cartCount}
                    ...
                    <div className="container mx-auto px-4">
                        <div className="flex items-center justify-between py-1.5 overflow-hidden">
                            <div className="flex items-center gap-6 overflow-x-auto text-sm font-medium scrollbar-hide">
                                ...
                                {['Sale', 'Best Seller', 'New'].map((item) => (
                                    <button
                                        key={item}
                                        onClick={() => {
                                            const tag = item === 'Sale' ? 'Sale' : item;
                                            navigate(`/products?tag=${tag}`);
                                        }}
                                        className="hover:text-yellow-400 transition-colors whitespace-nowrap text-white no-underline bg-transparent border-none cursor-pointer text-sm font-medium"
                                    >
                                        {item}
                                    </button>
                                ))}
